...
private:
  ...
  void _buildDelegate()
  {
    ...
    else if (delegateKind == "month")
    {
      ...
      addChild(mi);
      applyAttachedUserAgentStyle(mi);
      mMonthInput = mi;
      if (!mPendingValue.empty()) glint_month_input_delegate_set_value(mMonthInput, mPendingValue);
      if (mFocusPending && mRoot)
      {
        mFocusPending = false;
        mRoot->SetFocus(mMonthInput);
    ...
    else if (delegateKind == "time")
    {
      ...
      mTimeInput = ti;
      if (!mPendingValue.empty()) glint_time_input_delegate_set_value(mTimeInput, mPendingValue);
      if (mFocusPending && mRoot)
      {
        mFocusPending = false;
        mRoot->SetFocus(mTimeInput);
